...
percentile_975 = qt(??????, df = deg, lower.tail = TRUE, log.p = FALSE)
...
percentile_95 = qt(??????, df = deg, lower.tail = TRUE, log.p = FALSE)
...
## 5% significant test
quantile_975 = qt(??????, df = deg, lower.tail = TRUE, log.p = FALSE)
t_stat > quantile_975
## p-value
pvalue = 2*pt(??????, df = deg, lower.tail = F, log.p = FALSE)
...
percentile_975 = qnorm(??????, lower.tail = TRUE, log.p = FALSE)
CI_right = ?????? +  se*??????
CI_left = ?????? -  se*??????
...
percentile_995 = qnorm(??????, lower.tail = TRUE, log.p = FALSE)
CI_right = ?????? +  se*percentile_995
...
beta1hat = 609.5
#percentile_995 = qt(0.995, df = deg, lower.tail = TRUE, log.p = FALSE)
percentile_95 = qnorm(??????, lower.tail = TRUE, log.p = FALSE)
...
#####################################
#select the subset
rowIndex = which(df['Sex']==?????)
rowIndex = which( df['Sex']== 0 )
df_female = df[rowIndex, ]
# training the model
linearMod <- ??????
linearMod <- lm(Earnings ~ ??????, data=df_female)
summary(linearMod) 
