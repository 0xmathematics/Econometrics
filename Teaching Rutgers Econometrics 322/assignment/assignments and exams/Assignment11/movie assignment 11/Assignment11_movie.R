...
colnames(df)
# Q1
OLS_model_1 <- lm( log(assaults) ~ attend_v, data = df)
coeftest(OLS_model_1, vcov. = vcovHC, type = "const")
coeftest(OLS_model_1, vcov. = vcovHC, type = "HC0")
round(OLS_model_1$coefficients, 3 )

# Q2 90%CI
p = 1
n = nrow(df)
deg = n- p-1
threshold_t = qt(0.95,deg)
sd_hetro = sqrt( diag( vcovHC(OLS_model_1, type = 'HC0') ) )
betaOLS = OLS_model_1$coefficients
# 99% CI hetero and t critical
left = betaOLS - threshold_t * sd_hetro
right = betaOLS + threshold_t * sd_hetro
CI90 = cbind(left,right)
round(CI90,3)

#CI homo
confint(OLS_model_1, vcov = vcovHC(OLS_model_1, type = 'const'), level = 0.90)
#!!!!!!!!!! CI heter command not valid !!!!!!!!!!!!!!!!!!!
confint(OLS_model_1, vcov = vcovHC(OLS_model_1,type = "HC0"), level = 0.90)

# Q3 seasonal effect of assaults: estimation
OLS_model_1 <- lm( log(assaults) ~ year2 +year3 +year4 +year5 +year6 +year7 +year8 +year9+year10+
                     month2 + month3+ month4+ month5+ month6+ month7+ month8+ month9+ month10+ month11+month12, data = df)
summary(OLS_model_1)

# Q4 seasonal effect of assaults: inference
####################
# Joint Test: LR Principle
####################
# F stat on all month ==0
SSR_U = sum( ( OLS_model_1$residuals )^2 )
OLS_model_1_restricted <- lm( log(assaults) ~ year2 +year3 +year4 +year5 +year6 +year7 +year8 +year9+year10+
                              , data = df)
...
start_IX = which(colnames(df) == "year2" )
end_IX = length(colnames(df))
X_names = colnames(df)[start_IX:end_IX ]
delete_IX1 = which(X_names == "year1" )
delete_IX2 = which(X_names == "month1" )
...
# Q5 seasonal effect of attend: estimation
OLS_model_3 <- lm( (attend_v+attend_m+attend_n) ~ year2 +year3 +year4 +year5 +year6 +year7 +year8 +year9+year10+
                     month2 + month3+ month4+ month5+ month6+ month7+ month8+ month9+ month10+ month11+month12, data = df)
summary(OLS_model_3)
...
# Q6 seasonal effect of attend: inference
####################
# Joint Test: LR Principle
####################
# F stat on all month ==0
SSR_U = sum( ( OLS_model_3$residuals )^2 )
OLS_model_3_restricted <- lm( (attend_v+attend_m+attend_n)  ~ year2 +year3 +year4 +year5 +year6 +year7 +year8 +year9+year10
                                , data = df)
SSR_R = sum( ( OLS_model_3_restricted$residuals )^2 )
n = nrow(df)
d = 11
p = 20
f = (SSR_R-SSR_U)/d / ( SSR_U/(n-p-1) )
...
# Joint Test: Wald Principle
####################
betaOLS = as.matrix(OLS_model_3$coefficients )
...
p = nrow(betaOLS)-1
d = 11
sigmasqaureHat = sum(OLS_model_3$residuals^2)/(n-p-1)
# Conventional H_0:  month1 = 0, month2 = 0, ..., month11 = 0
A_1 = matrix(rep(0,d*(p+1-d)), nrow = d,ncol = 10 )
A_2 = diag(d)
A = cbind(A_1, A_2)
...
# Q6 assult on movie attandance and controls: estimation
colnames(df)
OLS_model_4 <- lm( log(assaults) ~ attend_v+attend_m+attend_n+
...
summary(OLS_model_4)
coeftest(OLS_model_4, vcov. = vcovHC, type = "const")
coeftest(OLS_model_4, vcov. = vcovHC, type = "HC0")
round(OLS_model_4$coefficients,3 )
